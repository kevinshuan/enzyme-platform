...
def generate_candidates(
    ...
    if mutation_rate == 0.0:
        logger.info(
            '{"event": "generate", "zero_rate": true, "n_candidates": %d, '
            '"timestamp": "%s"}',
            n_candidates,
    ...
    while len(candidates) < n_candidates:
        ...
        # Generate a deterministic UUID from the seeded rng so that
        # the same seed always produces the same candidate ids (US3).
        id_bytes = rng.integers(0, 256, size=16, dtype=np.uint8)
    ...
    logger.info(
        '{"event": "generate", "n_candidates": %d, "mutation_rate": %s, '
        '"timestamp": "%s"}',
        n_candidates,
        mutation_rate,
        datetime.now(timezone.utc).isoformat(),
